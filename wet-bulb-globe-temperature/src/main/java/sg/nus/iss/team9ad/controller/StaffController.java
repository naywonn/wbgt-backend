...
public class StaffController {
	...
	@PostMapping("/login")
	public ResponseEntity<String> login(@RequestBody Staff inStaff, HttpSession session) {
		// 在实际应用中，根据员工ID查找员工并验证密码
		
		Optional<Staff> authenticatedStaffOpt= staffService.findStaff(inStaff.getId());

		if (authenticatedStaffOpt.isPresent()) {
			Staff authenticatedStaff = authenticatedStaffOpt.get();
			if (authenticatedStaff.getPassword().equals(inStaff.getPassword())) {
				// 将认证后的员工信息保存到会话中
